...
class Task(db.Model):
    ...
    def __init__(self,Task_id,Title,Description,Priority,Planned_Date,Assignee,Reporter,Status,Team_Name,User_Name):
    ...
    def json(self):
        return {'Task_id':self.Task_id,'User_Name': self.User_Name, 'Title': self.Title,'Description': self.Description,'Priority': self.Priority,'Assignee':self.Assignee,'Reporter':self.Reporter,'Status':self.Status,'Team_Name':self.Team_Name}
...
class CreateTask(Resource):
    ...
    def post(self):
        ...
        task=Task(None,data['Title'],data['Description'],data['Priority'],data['Planned_Date'],data['Assignee'],data['Reporter'],data['Status'],data['Team_Name'],data['User_Name'])
        ...
        return {'Task_id':cur_task.Task_id, 'Title': cur_task.Title,'Description':cur_task.Description,'Priority': cur_task.Priority,'Planned_Date':val,'Assignee':cur_task.Assignee,'Reporter':cur_task.Reporter,'Status':cur_task.Status,'Team_Name':cur_task.Team_Name,'User_Name': cur_task.User_Name}, 201
...
class PutStatus(Resource):
    ...
    def patch(self, Task_id):
        ...
        if task:
            ...
            task.save_to_db
            return task.json()
        return {'message':'Task_id Doesnt exist'}
    @jwt_required()
